...
final class Potrace {
    ...
    class Curve {
        ...
        init(n: Int) {
            self.n = n
            tag = [String](repeating: "", count: n)
            c = BoxedArray(repeating: Point(x: 0.0, y: 0.0), count: n * 3)
            alphacurve = 0
            vertex = BoxedArray(repeating: Point(x: 0.0, y: 0.0), count: n)
            alpha = [Double](repeating: 0.0, count: n)
            alpha0 = [Double](repeating: 0.0, count: n)
            beta = [Double](repeating: 0.0, count: n)
        }
    }
    ...
    func processPath() {
        struct Quad {
            ...
            init() {
                data = [Double](repeating: 0.0, count: 9)
            }
        ...
        func calcLon(path: Path) {
            ...
            var i: Int, j: Int, k1: Int, a: Int, b: Int, c: Int, d: Int, k = 0
            ...
            for i in (0 ... n - 1).reversed() {
                ...
                ct[3] = 0
                dir = (
                    3 + 3 * (pt[mod(a: i + 1, n: n)].x - pt[i].x) +
                        (pt[mod(a: i + 1, n: n)].y - pt[i].y)
                ) / 2
                ct[dir] += 1
                ...
                while true {
                    foundk = 0
                    dir = (
                        3 + 3 * sign(i: pt[k].x - pt[k1].x) +
                            sign(i: pt[k].y - pt[k1].y)
                    ) / 2
                    ct[dir] += 1
                    ...
                    if xprod(p1: constraint[0], p2: cur) < 0 || xprod(p1: constraint[1], p2: cur) >
                        0
                    {
                        break
                    }
        ...
        func adjustVertices(path: Path) {
            ...
            for i in 0 ..< m {
                ...
                while true {
                    ...
                    if det != 0.0 {
                        w.x = (-Q.at(x: 0, y: 2) * Q.at(x: 1, y: 1) + Q.at(x: 1, y: 2) * Q.at(
                            x: 0,
                            y: 1
                        )) / det
                        w.y = (Q.at(x: 0, y: 2) * Q.at(x: 1, y: 0) - Q.at(x: 1, y: 2) * Q.at(
                            x: 0,
                            y: 0
                        )) / det
                        break
        ...
        func smooth(path: Path) {
            ...
            for i in 0 ..< m {
                ...
                if denom != 0.0 {
                    dd = dpara(p0: curve.vertex[i], p1: curve.vertex[j], p2: curve.vertex[k]) /
                        denom
                    dd = abs(dd)
        ...
        func optiCurve(path: Path) {
            class Opti {
                ...
                init() {
                    pen = 0
                    c = BoxedArray(repeating: PointF(x: 0, y: 0), count: 2)
                    t = 0
                    s = 0
                    alpha = 0
                }
            ...
            var curve = path.curve, m = curve.n, vert = curve.vertex,
                pt = [Int](repeating: 0, count: m + 1),
                pen = [Double](repeating: 0, count: m + 1),
                len = [Int](repeating: 0, count: m + 1),
                opt = [Opti](repeating: Opti(), count: m + 1),
                om: Int, i: Int, j: Int, r: Int,
                o = Opti(), p0: PointF,
                i1: Int, area: Double, alpha: Double, ocurve: Curve,
                s: [Double], t: [Double]

            var convc = [Int](repeating: 0, count: m),
                areac = [Double](repeating: 0, count: m + 1)

            for i in 0 ..< m {
                if curve.tag[i] == "CURVE" {
                    let val = dpara(
                        p0: vert[mod(a: i - 1, n: m)],
                        p1: vert[i],
                        p2: vert[mod(a: i + 1, n: m)]
                    )
            ...
            for j in 1 ... m {
                ...
                while i >= 0 {
                    r = opti_penalty(
                        ...
                        j: mod(a: j, n: m),
                        res: o,
                        opttolerance: info.opttolerance,
                        convc: convc,
                        areac: areac
                    )
                    if r == 1 {
            ...
            om = len[m]
            ocurve = Curve(n: om)
            s = [Double](repeating: 0, count: om)
            t = [Double](repeating: 0, count: om)
